...
var defaults = {
	...
	useStyleGraveAccent: true,
	customFilePath: defaultCustomFilePath,
	supportNonExistentFiles: false
};

function defaultProcessor(path, ext, file, url, cb) {
	return cb(null, file);
}
...
module.exports = function parser(file, options) {
	...
	function replaceFrag(cb) {
		...
		function _replaceFrag(cb) {
			...
			var styleGraveAccent = opts.useStyleGraveAccent ? '`' : '';
			// Build the final string.
			if ('html' === opts.type)
				assetFiles = opts.prop + ': `' + startOfInsertionBlock + assetFiles + endOfInsertionBlock + indentation + '`';
			if ('css' === opts.type)
				assetFiles = opts.prop + ': [' + styleGraveAccent + startOfInsertionBlock + assetFiles + endOfInsertionBlock + indentation + styleGraveAccent + ']';
				...
				return function (cb) {
					...
					if (HTML && opts.templateProcessor) {
						process.nextTick(function () {
							opts.templateProcessor(path, ext, data, url, customProcessorCallback(cb));
						});
					}
					if (CSS && opts.styleProcessor) {
						process.nextTick(function () {
							opts.styleProcessor(path, ext, data, url, customProcessorCallback(cb));
						});
					}
				};
			}), cb);
